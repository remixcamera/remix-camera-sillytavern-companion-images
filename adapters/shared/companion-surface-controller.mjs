...
const WRAPPER_COMMANDS = new Set(["remix", "photo", "image"]);
...
export function isExplicitCompanionImageRequest(text) {
  ...
  return includesAny(lower, [
    /\b(selfie|photo|picture|pic|image|snap|shot|portrait)\b/,
    /\bshow me\b/,
    /\bsend (me )?(one|it|that|a|an|some)?\b/,
...
function consentForCommand(command) {
  return ["couple-photo", "couples-vacation"].includes(command) ? "yes" : undefined;
}
...
export function parseNaturalCompanionImageRequest(text, action = "generate") {
  ...
  const parsed = {
    ...
    text: cleaned,
    natural: true,
    userConsent: consentForCommand(command),
  };
  ...
  if (WRAPPER_COMMANDS.has(commandName)) {
    ...
    if (explicitCommand) {
      return {
        ...
        text: nestedRest.join(" ").trim(),
        userConsent: consentForCommand(explicitCommand),
      };
    }
...
export function buildCompanionBridgeInput(parsed, options = {}) {
  ...
  const input = {
    ...
    userConsent: parsed?.userConsent || (isCoupleCommand ? "yes" : undefined),

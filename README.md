...
- `adapters/`: wrappers for MCP clients, RisuAI, Open WebUI, LibreChat, LobeChat, Agnai, Telegram, Discord, WhatsApp, Slack, LINE, Messenger, Instagram DMs, Microsoft Teams, Twilio SMS/MMS, Matrix, Dify, Flowise, Botpress, AnythingLLM, TypingMind, Poe, Langflow, LangChain JS, and the Vercel AI SDK.
...
- MCP clients, AnythingLLM, TypingMind, Poe, Langflow, LangChain JS, and the Vercel AI SDK: supported through local stdio MCP, official custom skill, plugin, server-bot, custom component, or framework tool surfaces.

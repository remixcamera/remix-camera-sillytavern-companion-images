...
const COMMON_PROPERTIES = {
  ...
  sourceImageUrl: {
    type: "string",
    format: "uri",
    description: "Public source image URL for outfit/reference/image-to-image workflows.",
  },
  theme: {
    type: "string",
  ...
  userReferenceImageKey: {
    type: "string",
    description: "Remix.Camera uploaded reference image key for the user in couple images.",
  },
  ...
  snapTtlSeconds: {
    ...
    minimum: 5,
    maximum: 86400,
    description: "Suggested display TTL for private snaps in clients that support ephemeral UI.",
  },
};
...
export function createBridgeOpenApiDocument(baseUrl = "http://127.0.0.1:8787") {
  const paths = {
    "/health": {
      get: {
  ...
  for (const command of COMPANION_COMMANDS) {
    paths[`/v1/tools/${command.name}/dry-run`] = {
      post: {
    ...
    paths[`/v1/tools/${command.name}/generate`] = {
      post: {
  ...
  return {
    ...
    info: {
      ...
      version: "0.4.0",
      description: "Companion-image tools backed by Remix.Camera prompt templates and a local scoped bridge token.",
    },
    servers: [
      {
        url: baseUrl,
...
export function createLobeManifest(baseUrl = "http://127.0.0.1:8787") {
  return {
    ...
    version: "1",
    api: COMPANION_COMMANDS.map((command) => ({
